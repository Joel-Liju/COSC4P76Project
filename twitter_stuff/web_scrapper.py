...
def extract_data_from_current_tweet_card(card):
    ...
    try:
        ...
        if 'Replying to' in _comment:
            _comment = card.find_element_by_xpath('.//div[2]/div[2]/div[2]').text
    except exceptions.NoSuchElementException:
        _comment = ""
...
def main(username, password, search_term, filepath, page_sort='Latest'):
    ...
    url = 'https://twitter.com/search?lang=en&q=(%40JustinTrudeau)%20lang%3Aen%20until%3A2020-02-29%20since%3A2020-02-28&src=typed_query'
    driver.get(url)
    sleep(2)
    ...
    while True:
        ...
        for card in cards:
            ...
            if counter > 250 or end_of_scroll_region:
                sys.exit(80)
                # day_num += 1
                # day_string = "{:02d}".format(day_num)
                # day2_string = "{:02d}".format(day_num+1)
                # month_string = "{:02d}".format(month)
                # month2_string = month_string

                # if month == 12 and day_num == 31:
                #     sys.exit(0)
                #
                # if (month % 2 == 1 and day_num == 30) or (month % 2 == 0 and day_num == 31) or (month == 2 and day_num == 28):
                #     day2_string = "01"
                #     month2_string = "{:02d}".format(month + 1)
                #     month += 1
                #     day_num = 1

                # url = 'https://twitter.com/search?lang=en&q=(%40realDonaldTrump)%20lang%3Aen%20until%3A2020-{}-{}%20since%3A2020-{}-{}&src=typed_query'.\
                #     format(month2_string, day2_string, month_string, day_string)

                # if month > 13:
                #     sys.exit(100)
                #
                # month += 1
                # url = 'https://twitter.com/search?lang=en&q=(%40realDonaldTrump)%20lang%3Aen%20until%3A2019-{}-02%20since%3A2019-{}-01&src=typed_query'.\
                #     format(month, month)
                #
                # if month > 12:
                #     url = 'https://twitter.com/search?lang=en&q=(%40realDonaldTrump)%20lang%3Aen%20until%3A2020-01-01%20since%3A2019-12-31&src=typed_query'
